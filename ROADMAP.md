...
- **思考内容独立视觉样式**
  - `stream:chunk` 中 `<think>...</think>` 标签由前端解析分离，渲染为 `.msg.think` 独立气泡
  - 灰色文字 + 斜体 + 左侧淡灰边框，与 assistant 回答明显区分
  - 思考内容可渲染 Markdown（代码块、段落等保持 dim 色调）
  - 流式渲染中实时更新，`</think>` 到达后自动切换回正常输出管道
  - 隐藏 thinking 动画点（真实内容替换占位符）
